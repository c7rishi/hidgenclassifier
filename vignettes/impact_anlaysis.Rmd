...
The impact mutation annotation dataset (stored as a `data.table` object) looks like this:
```{r, view_data}
impact
```
...
We want to train a hidden genome (multinomial logistic) classifier with these 10 cancer sites (response classes) using the variants listed in column "Variant" as predictor while simultaneously utilizing the meta-features gene (column "Hugo_Symbol") and 96 Single Base Substitution (SBS-96) categories. The sample/patient ids of tumors are listed in column "patient_id". Enter `?impact` in the R console to see more details on the dataset.
...
pid <- names(canc_resp)
```
To train the model, we first split the dataset into 5 stratified random folds, based the cancer categories. Then define our training set by combining four out of the five folds, and use the remaining fifth fold as our test set.
```{r train_test_pid}
set.seed(42)
...
With the most discriminative variants  determined and stored in `top_v`,  we now extract the variant design matrix X restricted to these variants for _all tumors_.  (The matrix will be row-subsetted to `pid_train` during training; the remaining rows will be used for prediction):
```{r design_variant}
